...
def tty_exit():
    ...
    if os.path.exists("/tmp/.SAM_tmp/TTY_cmd_pipe"):
        subprocess.run(["tmux", "kill-session", "-t", "OLED"])
    ...
    while True:
        try:
            ...
            if not state.is_sam_running():
                ...
                if can_start and 0 < time_left <= 5:
                    # Start countdown if we are within 5 seconds of timeout
