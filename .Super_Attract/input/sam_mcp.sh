...
function sam_trigger() {
	...
	input_clear	
		if [ "$(ps aux | grep -ice "[M]iSTer_SAM_on")" == "0" ] && [ "$(ps aux | grep -ice [u]pdate)" == "0" ]; then
			...
			/media/fat/Scripts/Super_Attract_Mode_on.sh start
...
function sam_jsmonitor() {
	...
	for joystick in ${jsnew[@]}; do
		if [ ! -z "${joystick}" ]; then
			...
			"${mrsampath}/input/sam_joy.py" "${joystick}" 2>/dev/null &
		fi
	done
	shopt -u nullglob
}
	...
	for f in "${rawinput[@]}"; do
		...
		for i in "${kbddev[@]}"; do
			if [ "$i" == "$rawdev" ] ; then
				...
				"${mrsampath}/input/sam_keyboard.py" "${f}" 2>/dev/null &
			fi
		done	
	done
}
...
[[ -z $(ps -o pid,args | grep '[M]iSTer_SAM_mouse.sh' | awk '{print $1}' | head -1) ]] && "${mrsampath}/input/sam_mouse.sh" 2>/dev/null &
