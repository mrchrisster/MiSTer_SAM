...
## Installation
- Copy `MiSTer_SAM.sh` to your MiSTer's `/media/fat/Scripts` directory - *that's it!* 
- From the main MiSTer menu open the OSD (F12 or your controller's menu button).
- Navigate to **Scripts** and select **MiSTer_SAM.sh**.
- NOTE: Additional files will be downloaded automatically the first time you run `MiSTer_SAM.sh`.
  
## Offline Installation  
If your MiSTer is not connected to the internet, click on "Code" -> "Download Zip" and download the project package.
- Copy `MiSTer_SAM.sh` and `MiSTer_SAM.ini` to `/media/fat/Scripts` on your MiSTer. 
...
Don't want to wait? You can start SAM instantly from `MiSTer_SAM.sh`!

## Updates
`MiSTer_SAM.sh` can update all its files to the latest release. Select "Update" from the menu and it will update all MiSTer_SAM files - except your `MiSTer_SAM.ini`.
  
- From the main MiSTer menu open the OSD (F12 or your controller's menu button).
- Navigate to **Scripts** and select **MiSTer_SAM.sh**.
- Select "Update" from the MiSTer SAM menu.
...
### Exclude
Want to exclude certain games? Just add them to the appropriate system `exclude` list in the `MiSTer_SAM.ini` file.
...
### Do you support SNAC?  
SNAC is not handled by the Linux kernel so unfortunately we cannot be able to support SNAC devices.
  
### I hate game `X`. How do I make sure SAM never shows it to me?
Look at the system specific exclude lists in the INI. Note you need to provide the complete name of the ROM, such as `Tetris World (USA).gba`.
...
### What was that game a few minutes ago? How do I know what game is currently playing?
SAM keeps a history of the games it loads in `/tmp/SAM_Games.log`. If you would like to know only which game is *currently* playing, you can check the file `/tmp/SAM_Game.txt`. Some folks even use this with OBS to automatically change the game name for their Twitch stream! These are accessible using ssh while SAM is running.
